          ...
          <div style={{ marginBottom: 24 }}>
            {setMastery.map(({ set, mastery, sessionCount }) => (
              <div key={set.id} style={{ marginBottom: 14 }}>
                ...
                <div style={S.bar}><div style={{ height: '100%', borderRadius: 3, background: mastery >= 0.7 ? '#34d399' : mastery >= 0.5 ? '#f59e0b' : theme.primary, width: `${mastery * 100}%`, transition: 'width 0.4s ease' }} /></div>
              </div>
            ))}
          </div>
        </>
              ...
              {recentSessions.map((s) => {
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td style={{ ...S.td, color: theme.primary }}>{s.mode.replace('_', ' ')}</td>
